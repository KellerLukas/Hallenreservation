...
class FindAttachmentMeta:
  ...
  def find(self, attachment_name: str, attachment_content: str) -> AttachmentMeta:
    prompt = self._setup_prompt(attachment_name=attachment_name, attachment_content=attachment_content)
    ...
    response.clean_filename = response.clean_filename if not is_none(response.clean_filename) else None
    ...
    logging.info(f"Found meta {response.clean_filename}, year {response.year} for for attachment {attachment_name}")
    return response
    
  def _setup_prompt(self, attachment_name: str, attachment_content: str) -> str:
    template = attachment_prompt_tmeplate
    prompt = template.format(mail_body=self.message_body, mail_subject=self.message_subject, attachment_name=attachment_name, attachment_content=attachment_content)
    return prompt
